...
import { Gamepad2, MessageCircle, Send, Users } from 'lucide-react';
...
export default function MessagesPage() {
  ...
  return <section className="mx-auto mb-8 max-w-6xl">
    <div className="mb-5"><p className="mb-1 text-xs font-semibold uppercase text-brand-violet">Conversaciones</p><h2 className="text-2xl font-bold text-white">Mensajes</h2></div>
    <div className="grid min-h-[650px] overflow-hidden rounded-xl border border-white/10 bg-[#0b111d] md:grid-cols-[300px_1fr]">
...
function ConversationButton({ conversation, active, onClick }: { conversation: Conversation; active: boolean; onClick: () => void }) {
  return <button onClick={onClick} className={`flex w-full items-center gap-3 border-b border-white/[0.05] px-4 py-3 text-left ${active ? 'bg-brand-violet/10' : 'hover:bg-white/[0.035]'}`}><img src={conversation.participant.avatar} alt="" className="h-11 w-11 rounded-lg object-cover" /><span className="min-w-0 flex-1"><span className="flex items-center justify-between gap-2"><strong className="truncate text-sm text-white">{conversation.participant.username}</strong>{conversation.unread > 0 && <span className="grid min-w-5 place-items-center rounded-full bg-brand-violet px-1.5 text-[10px] font-bold text-white">{conversation.unread}</span>}</span><small className="mt-0.5 block truncate text-slate-500">{conversation.kind === 'match' ? 'Match · ' : ''}{conversation.lastMessage}</small></span></button>;
}
