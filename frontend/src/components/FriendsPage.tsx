  ...
  return <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/75 p-3 sm:p-6" onClick={onClose}>
    <div role="dialog" aria-modal="true" aria-labelledby="friend-profile-title" className="glass max-h-[92vh] w-full max-w-4xl overflow-y-auto rounded-xl" onClick={(event) => event.stopPropagation()}>
      ...
      <div className="p-5 sm:p-6">
        {loading && <div className="flex items-center gap-2 py-8 text-sm text-slate-400"><Loader2 className="h-5 w-5 animate-spin text-brand-violet" /> Cargando perfil...</div>}
        {!loading && <>
          ...
          <section className="border-b border-white/10 py-6">
            ...
            {dashboardLoading ? <div className="flex items-center gap-2 py-10 text-sm text-slate-400"><Loader2 className="h-5 w-5 animate-spin text-brand-violet" /> Consultando Riot Games...</div> : dashboardError ? <div className="mt-5 border-l-2 border-amber-400/60 py-2 pl-4 text-sm text-slate-400">{dashboardError}</div> : dashboard && <>
              <div className="mt-6 grid grid-cols-2 border-y border-white/10 sm:grid-cols-4">
                ...
                <ProfileStat icon={Swords} label="KDA" value={String(recent?.kda ?? 0)} detail="Últimas 5" />
                <ProfileStat icon={BarChart3} label="CS promedio" value={String(recent?.cs ?? 0)} detail="Por partida" />
              </div>
