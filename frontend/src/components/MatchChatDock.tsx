...
import { ArrowSquareOut, CaretDown, CaretUp, ChatCircle, Check, Copy, DiscordLogo, PaperPlaneTilt, X } from '@phosphor-icons/react';
  ...
  return <aside className={`relative w-[min(350px,calc(100vw-2rem))] shrink-0 overflow-hidden rounded-t-xl border border-white/10 bg-[#0b111d] shadow-2xl transition-all ${minimized ? 'h-14' : 'h-[480px]'}`}>
    ...
    {!minimized && <div className="flex h-[426px] flex-col">
      {status.discordInviteUrl && (
        <div className="flex items-center gap-3 border-b border-[#5865F2]/25 bg-[#5865F2]/12 px-4 py-2.5">
          <DiscordLogo weight="fill" className="h-4 w-4 shrink-0 text-[#9da6ff]" />
          ...
          <a
            ...
            target="_blank"
            rel="noreferrer"
            className="inline-flex min-h-8 items-center gap-1.5 rounded-md bg-[#5865F2] px-2.5 text-xs font-semibold text-white hover:bg-[#6773f4]"
          >
            Unirme <ArrowSquareOut className="h-3.5 w-3.5" />
          </a>
        </div>
      )}
